...
def sigmoid(x):
    """Cette fonction calcule la valeur de la fonction sigmoide (de 0 à 1) pour un nombre réel donné. Il est à noter
    que x peut également être un vecteur de type numpy array, dans ce cas, la valeur de la sigmoide correspondant à
    chaque réel du vecteur est calculée. En retour de la fonction, il peut donc y avoir un objet de type
    numpy.float64 ou un numpy array. """
    return 1 / (1 + np.exp(-x))


def initWeights(nb_rows, nb_columns):
    """Fonction destinée à initialiser les poids d'une matrice de genre (nb_rows * nb_columns) pour un réseau de
    neurones sur base d'une distribution normale de moyenne 0 et d'un écart-type de 0.0001. """
    return np.random.normal(0, 0.0001, (nb_rows, nb_columns))


def createNN(n_input, n_hidden):
    """Fonction permettant de créer un réseau de neurones en fonction de la taille de la couche d'entrée (n_input) et
    de la couche intermédiaire (n_hidden) Le réseau de neurones créé est ensuite retourné sous la forme d'un tuple de
    2 numpy array contenant respectivement les coefficients de poids liant la couche d'entrée à la couche
    intermédiaire et les coefficients de poids liant la couche intermédiaire à la couche de sortie.
    """
    W_int = initWeights(n_hidden, n_input)
    W_out = initWeights(n_hidden, 1)[:, 0]
    # W2 est traité comme vecteur et non comme une matrice Hx1 (simplifie certaines ecritures)
    return W_int, W_out
...
def forwardPass(s, NN):
    ...
    joueur blanc pour un état (s) donné du jeu. """
    W_int = NN[0]
    W_out = NN[1]
...
def backpropagation(s, NN, delta, learning_strategy=None):
    ...
    """
    # remarque : les operateurs +=, -=, et *= changent directement un numpy array sans en faire une copie au 
    # prealable, ceci est nécessaire lorsqu'on modifie W_int, W_out, Z_int, Z_out ci-dessous (sinon les changements 
    # seraient perdus après l'appel) 
...
def makeMove(moves, s, color, NN, eps, learning_strategy=None):
    ...
    if greedy or Q_learning:
        best_moves = []
        best_value = None
        c = 1 if color else -1  # reversed, 0 is black player!
        for m in moves:
