...
class GameEngine(QObject):
    ...
    :param game_parameters: {mode: match | train | compare,
                             player1: {type: human | random | minimax | ml agent,
                                       pars: {network_name, eps, [ls, lr, eps_dec, act_f] | ehits, eps}
                                        },
        ...
        if self.game_parameters["mode"] == "match" or self.game_parameters["mode"] == "compare":
            for role, player in enumerate(("player1", "player2")):
                if self.game_parameters[player]["type"] == "human":
                    ...
                    self.game_parameters[player]["pars"]["act_f"], w_int, w_out = load_network(self.game_parameters[player]["pars"]["network_name"])
                    network = w_int, w_out
            ...
            self.game_parameters["player1"]["pars"]["act_f"], w_int, w_out = load_network(self.game_parameters["player1"]["pars"]["network_name"])
            network = w_int, w_out
    ...
    def match_loop(self):
        ...
        while not end:
            ...
            if not isinstance(cur_player, PlayerHuman):
                time.sleep(1.2)  # short break before AI move

            # check if available move
            new_move = self.game.player_has_move(cur_player.get_role())
            if new_move:
                new_move = None
